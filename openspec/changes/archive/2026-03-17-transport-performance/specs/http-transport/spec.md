...
- **THEN** system creates a default `http.Client` with optimized settings:
  - MaxIdleConns: 100
...
- **THEN** system configures reasonable defaults for connection pooling and TLS with performance optimizations

### Requirement: Automatic retry with exponential backoff
The system SHALL automatically retry failed requests using exponential backoff with jitter, with buffer pooling to reduce allocations.
...
The system SHALL enforce request timeouts at the HTTP transport layer with optimized buffer management.
...
The system SHALL handle HTTP request creation and response parsing with buffer pooling to reduce allocations.

#### Scenario: JSON request body
- **WHEN** request includes a body struct
- **THEN** system marshals the body to JSON using pooled buffers and sets `Content-Type: application/json`

#### Scenario: JSON response parsing
- **WHEN** response has `Content-Type: application/json`
- **THEN** system unmarshals the response body into the target struct using efficient decoding
...
#### Scenario: Error body size limiting
- **WHEN** response has error status code (4xx or 5xx)
- **THEN** system reads at most 1MB of response body to prevent memory spikes

### Requirement: TCP connection warming
The system SHALL implement TCP connection warming to reduce TTFT with optimized transport settings.
...
### Requirement: Buffer pooling
The system SHALL use `sync.Pool` for request/response buffers to reduce memory allocations.

#### Scenario: Buffer reuse for requests
- **WHEN** making multiple requests
- **THEN** system reuses buffers from pool instead of allocating new ones

#### Scenario: Buffer reset between uses
- **WHEN** a buffer is retrieved from the pool
- **THEN** system calls `Reset()` before using to prevent data leakage

#### Scenario: Buffer return after use
- **WHEN** request/response processing is complete
- **THEN** system returns buffer to pool for reuse

### Requirement: Streaming optimization
The system SHALL use optimized buffer sizes for SSE streaming to improve throughput.

#### Scenario: Large streaming buffer
- **WHEN** reading SSE stream
- **THEN** system uses 32KB buffer size for efficient reading

#### Scenario: Reduced system calls
- **WHEN** processing streaming responses
- **THEN** system amortizes system calls over larger buffer reads
